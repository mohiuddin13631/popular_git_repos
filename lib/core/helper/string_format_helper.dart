...
class AppConverter {
  ...
  static String replaceUnderscoreWithSpace(String value) {
    try {
      ...
      String v = formatedString.split(" ").map((str) => str.toCapitalized()).join(" ");
      return v;
    } catch (e) {
      return value;
    }
  ...
  static String getFormattedDateWithStatus(String inputValue) {
    ...
    try {
      ...
      if (dayDef == 0) {
        if (hourDef <= 0) {
          if (minDef <= 0) {
            value = '$secondDef ${AppStrings.secondAgo}';
          } else {
            value = '$hourDef ${AppStrings.minutesAgo}';
          }
        } else {
          value = '$hourDef ${AppStrings.hourAgo}';
        }
      } else {
        value = '$dayDef ${AppStrings.daysAgo}';
      }
    } catch (e) {
      value = inputValue;
    }
...
extension StringCasingExtension on String {
  ...
  String cleanAndTitleCase() {
    return replaceAll('_', ' ') // Step 1: Replace _ with space
        .replaceAll(RegExp(r'[^a-zA-Z0-9 ]'), '') // Step 2: Remove special characters except space
